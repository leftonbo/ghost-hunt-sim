...
| `TongueGhost` | `src/entities/TongueGhost.ts` | べろべろおばけ。遠距離から舌を伸ばしてホーミング捕食。舌にもランタン判定あり。緑系、螺旋瞳。移動速度遅い |
...
- べろべろ（`TONGUE_RANGE`, `TONGUE_EXTEND_SPEED`, `TONGUE_HOMING_STRENGTH`, `TONGUE_COOLDOWN`, `TONGUE_TIP_CAPTURE_DIST`）
