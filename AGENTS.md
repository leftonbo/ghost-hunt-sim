...
- タスクは `main` ブランチ以外で実施してください (例: `feature/add-feral-ghost`)
- タスク完了後は git コミットを行ってください
- Serena MCP が利用可能である場合:
  - Serena MCP から現状の把握やメモリの更新を行ってください
  - タスク完了後は Serena MCP にメモリの更新を行ってください
  - Serena のメモリのコミットは、本作業コミットとは別に行ってください
...
| クラス | ファイル | 責務 |
| ------ | -------- | ---- |
| `Ghost` | `src/entities/Ghost.ts` | おばけ基底クラス（ノーマル）。状態管理（hunting/digesting/stunned）、複数ニンゲン同時捕食（capturedHumans[]）、並列生気吸収（2人目以降MULTI_CAPTURE_DRAIN_MULTIPLIER減衰）、もがき判定、外部スタン（ランタン等で全員吐き出し）、convertedHumans[]による変換管理、描画。サブクラスがオーバーライド可能なupdateHunting/updateDigesting/updateStunned/drawBody/drawFace/getHealthDrainRate等のメソッドを持つ |
...
| `SmokeGhost` | `src/entities/SmokeGhost.ts` | けむりおばけ。ニンゲンより遅く、遠距離へ煙弾を投げて煙幕内のニンゲンに移動速度低下・スタミナ減少を与える。煙で弱ったニンゲンだけを接触捕食する。灰緑系、薄い煙をまとった外見 |
...
```text
hunting ←→ [ニンゲンに接触] → digesting（複数ニンゲン並列消化）
                                  ↓
              [生気0のニンゲン] → convertedHumans[] に移動（Simulationが新おばけ生成）
              [全員消化完了]   → hunting に復帰
                                  ↓
          [最後の1人が脱出成功] → stunned → [スタンタイマー完了] → hunting
                                  ↓
      [ランタン発動（外部スタン）] → stunned（全ニンゲン吐き出し）→ hunting
```

- `hunting`: 最寄りニンゲンを追跡（sin波で横揺れしながら移動）。消化中でも追加捕食可能（canCapture）
- `digesting`: ゆっくり浮遊、体が膨張（捕食数に応じて半径増加）、複数ニンゲンの生気を並列吸収
  - 2人目以降は `MULTI_CAPTURE_DRAIN_MULTIPLIER`（0.7）で吸収効率低下
  - 生気が0になったニンゲンは `convertedHumans[]` に移動（Simulationが新おばけを生成）
  - 全capturedHumansが空になり、脱出があった場合は `stunned` に遷移
  - 全capturedHumansが空になり、脱出がなかった場合は `hunting` に復帰
  - ランタン等の外部スタンで `stunned` に遷移（全ニンゲンを吐き出す）
- `stunned`: 最後の1人に脱出されて一定時間動けない（点滅表示）、タイマー完了で `hunting` に復帰
...
- **もがき（escapeProgress）**: 捕食中にスタミナを消費して蓄積。`ESCAPE_THRESHOLD` 到達で脱出成功。
  - スタミナ0の間はもがけず進捗停止。
- **無敵（invincibilityTimer）**: 脱出成功時または外部スタンで吐き出された時に `INVINCIBILITY_DURATION`（180フレーム = 3秒）が付与。
  - 無敵中は捕食・吸引プル・舌掴みが無効。
  - 点滅表示で視覚的に確認可能。
...
- 視界半径内のおばけを検知
- 検知した全おばけの反対方向の合成ベクトルに、横ステップ（サイドドッジ）を加えて逃走
- 角近傍では corneredness に応じて逃走加速度と分離力を強化
- 壁回避は角で相反する回避力の相殺を抑える重み調整を行う
- 壁反射後は内向きベクトル方向へ `wanderAngle` を再設定して角振動を抑える
...
- 複数捕食時の吸収効率減衰（`MULTI_CAPTURE_DRAIN_MULTIPLIER`）
...
- 逃走の加速度・横ステップ・緊急度減衰（`HUMAN_FLEE_ACCEL`, `HUMAN_FLEE_DODGE`, `HUMAN_ESCAPE_URGENCY_DECAY`）
- 角詰まり時の逃走ブースト（`CORNER_ESCAPE_ACCEL_BOOST`, `CORNER_SEPARATION_BOOST`）
- 角での壁回避重み（`CORNER_WALL_OPPOSING_DAMP`, `CORNER_WALL_AVOID_REDUCTION`）
- 壁反射の押し戻し（`WALL_BOUNCE_BOOST`, `WALL_BOUNCE_JITTER`）
...
- 脱出後無敵時間（`INVINCIBILITY_DURATION`）
...
- けむり（`SMOKE_SPEED_MULTIPLIER`, `SMOKE_THROW_RANGE`, `SMOKE_PROJECTILE_SPEED`, `SMOKE_CLOUD_RADIUS`, `SMOKE_CLOUD_DURATION`, `SMOKE_COOLDOWN`, `SMOKE_DEBUFF_DURATION`, `SMOKE_DEBUFF_SPEED_MULTIPLIER`, `SMOKE_DEBUFF_STAMINA_DRAIN_RATE`, `SMOKE_WEAK_HEALTH_RATIO`）
