...
| クラス | ファイル | 責務 |
| ------ | -------- | ---- |
| `Ghost` | `src/entities/Ghost.ts` | おばけエンティティ。状態管理（hunting/digesting/releasing/stunned）、最寄りニンゲン追跡、捕食・生気吸収、もがき判定、外部スタン（ランタン等）、描画 |
| `Human` | `src/entities/Human.ts` | ニンゲンエンティティ。生気・スタミナ管理、逃走行動、疲労状態、もがき（捕食中）、ランタン所持・ドロップ、ランダム移動、簡易Boids群れ行動、壁反射、描画 |
...
| `Simulation` | `src/entities/Simulation.ts` | メインシミュレーション管理。エンティティ生成/更新/描画、捕食判定、ランタン管理（拾得・発動・ドロップ）、終了判定、UI連携 |
